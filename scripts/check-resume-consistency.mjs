...
const FORBIDDEN = [
  'NexusAOS', 'nexus-aos', 'nexusaos', 'Nexus bridge',
  'nexus-kernel', 'nexus_kernel', 'SeshaOS', 'sesha-os', 'Sesha OS',
  '12 workspace crates', '12-crate workspace',
]
...
const tsSlugs = new Set(
  [...projectsTs.matchAll(/github:\s*'https:\/\/github\.com\/gaganjainse\/([^']+)'/g)].map((m) => m[1]),
)
...
for (const s of tsSlugs) if (!htmlSlugs.has(s)) fail(`projects.ts has "${s}" but resume.html is missing it`)
...
const cargoToml = await (await fetch(
  'https://raw.githubusercontent.com/gaganjainse/SheshAOS/main/Cargo.toml',
  { headers: authHeaders },
)).text()
