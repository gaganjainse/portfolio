...
const ALLOWLIST = new Set(
  [
    // names / places
    'gagan', 'jain', 'jaipur', 'vellore', 'codenplay', 'gaganjainse',
    // project & product names
    'sheshaos', 'shesh', 'vyakrti', 'vyākṛti', 'omniroute', 'seshaos', 'nexusaos',
    // frameworks / tools
    'litellm', 'ollama', 'langchain', 'llamaindex', 'fastapi', 'chromadb',
    'ragas', 'pytest', 'tokio', 'axum', 'ratatui', 'iced', 'tailwind', 'monaco',
    'zustand', 'gunicorn', 'nginx', 'laravel', 'supabase', 'mongodb', 'redis',
    'sqlite', 'docker', 'websockets', 'folium', 'leaflet', 'tkinter', 'pulp',
    'chart', 'fullcalendar', 'cachyos', 'hyprland', 'moondream', 'nomic',
    'embed', 'text', 'qwen', 'gemma', 'phi', 'lora', 'qlora', 'vram', 'vr',
    'vdir', 'ical', 'icalendar', 'vevent', 'vdirsyncer', 'khal', 'imap',
    'guardedmcp', 'mcp', 'a2a', 'pwa', 'rag', 'nlp', 'gpt', 'api', 'apis',
    'openai', 'anthropic', 'codesys', 'node', 'red', 'plc', 'hmi', 'zig',
    'argon', 'hsts', 'csrf', 'csp', 'jwt', 'prometheus', 'kubernetes', 'linux',
    'llms', 'ai', 'llm', 'genai', 'rpc', 'cli', 'tui', 'gui', 'ssh', 'pty',
    'vt100', 'bm25', 'rrf', 'arxiv', 'github', 'gitlab', 'vercel', 'linkedin',
    'i18n', 'aos', 'ide', 'lsp', 'repl', 'rtk', 'caveman', 'ngo', 'ngos',
    'blas', 'cuda', 'nim', 'lms', 'llmops', 'devops', 'backend', 'frontend',
    'genai', 'iast', 'sanskrit', 'transliteration',
    // build/test vocabulary
    'clippy', 'lexer', 'parser', 'bytecode', 'evals', 'eval', 'scorers', 'devel',
    'canary', 'containerized', 'repls', 'harness', 'golden', 'snapshot', 'lint',
    'gate', 'gates', 'manifest', 'manifests', 'orchestrator', 'audited',
    'reversible', 'permissioned', 'offline', 'multiplexing', 'telemetry',
  ].map((w) => w.toLowerCase()),
)
...
async function checkText(label, text) {
  for (const [i, chunk] of chunks(text).entries()) {
    ...
    for (const m of data.matches || []) {
      ...
      const ctx = chunk.slice(Math.max(0, m.offset - 40), m.offset + m.length + 40).replace(/\s+/g, ' ')
      console.log(`[${cat}] ${label}: ${m.message} → "…${ctx}…" (suggest: ${(m.replacements || []).slice(0, 3).map((r) => r.value).join(', ') || '—'})`)
      sawAny = true
...
const inputs = [
  ...
  [
    ...
    [...read('src/data/projects.ts').matchAll(/description:\s*'([^']+)'/g)].map((m) => m[1]).join('. '),
  ],
]
